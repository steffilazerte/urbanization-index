(script-fu-register
    "script-fu-habitat-resave"                        ;func name
    "Habitat 3 - RESave"                               ;menu label
...
(define (base-name orig-name divider)
  (car (reverse (strbreakup (car (strbreakup orig-name "_b.xcf")) divider)))
)


(script-fu-menu-register "script-fu-habitat-resave" "<Image>/Filters")

(define (script-fu-habitat-resave image drawable)
  (let* (
	 ;; Guess host OS based on directory path separator (based on script from http://www.gimp.org/tutorials/AutomatedJpgToXcf/)

	 ;; Get directory location
	 (local_dir (car (strbreakup (car (gimp-image-get-filename image)) "gimp")))
    ...
    (gimp-message (string-append "data file name: " output2))
    ...
    (gimp-message pixels-naturalgrass)
